...
function path = astar_search(map, start, goal)
    ...
    while ~isempty(open)
        ...
        for i = 1:size(neighbors, 1)
            ...
            % previous position은 parent node로부터 가져와
            pr = parent(current(1),current(2),1);
            pc = parent(current(1),current(2),2);
            
            if pr ~=0 && pc ~=0
                % previous direction
                prev_dir = [current(1)-pr,current(2)-pc];

                % current direction
                curr_dir = [r-current(1),c-current(2)];

                % 방향 cost
                cos_theta = dot(prev_dir,curr_dir) / (norm(prev_dir) * norm(curr_dir) + eps);
                angle_penalty = acos(cos_theta);
            ...
            tentative_cost = cost(current(1),current(2)) + move_cost + penalty_weight*angle_penalty;
